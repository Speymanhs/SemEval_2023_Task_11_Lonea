...
#_________________________________________________________________________________
train_ds, dev_ds = prepare_dataset(['MD-Agreement'], ['train', 'dev'])
...
def build_classifier_model():
  ...
  #net = step_sin_activation(net * 0.2, 6, 0.0)
...
with strategy.scope():
    ...
    #trained_model.load_weights('./models/evaluation_phase/exp_3/MD_Agreement_35_epochs_LinAct.h5')
...
checkpoint = tf.keras.callbacks.ModelCheckpoint('./models/evaluation_phase/exp_3/MD_Agreement_35_epochs_LinAct.h5', monitor="val_loss", mode="min", save_best_only=True, save_weights_only=1, verbose=1)
...
#_________________________________________________________________________________
trained_model = build_classifier_model()
trained_model.load_weights('./models/evaluation_phase/MD_Agreement_35_epochs_vanilla.h5')
optimizer = optimization.create_optimizer(init_lr=init_lr,
                                          num_train_steps=num_train_steps,
                                          num_warmup_steps=num_warmup_steps,
                                          optimizer_type='adamw')
...
with open("./tsv_results/MD-Agreement_results.tsv", "w") as record_file:
    ...
    for i in range(len(tf_dev_list)):
        writer.writerow(["%d" % (int(round(step_discrete_func(trained_model(tf.constant([tf_dev_list[i]])).numpy().astype(float)[0][0], 5, 1)))),
                        "%f" % (1.0 - step_discrete_func(trained_model(tf.constant([tf_dev_list[i]])).numpy().astype(float)[0][0], 5, 1)),
                        "%f" % step_discrete_func(trained_model(tf.constant([tf_dev_list[i]])).numpy().astype(float)[0][0], 5, 1)])
#_________________________________________________________________________________
